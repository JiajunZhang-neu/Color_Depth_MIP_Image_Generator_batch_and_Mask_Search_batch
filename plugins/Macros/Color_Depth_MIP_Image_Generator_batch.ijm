//Modified by Jiajun Zhang (University of Cologne), Apr 7, 2025
...
dirCOLOR = dirCOLOR + File.separator;
...
if(exi==1){
	...
	for (si=0; si<n; si++) {
		...
		}else if(swi==33 && swi<=testline){
			Hrizontal = String.buffer;
		}  //swi==0 //swi==0
	}
...
Dialog.addCheckbox("Horizontal CDM", Hrizontal);
...
easyADJ=true;
else if (AutoBRVST=="Segmentation based automatic" || AutoBRVST=="Segmentation based no lower value cut")
AutoBRV=1;
	...
	if(filepathcolor==1){
		...
		if(dotIndexJBA>-1 || dotIndexCMTK>-1 || dotIndextif>-1 || dotIndexTIFF>-1 || dotIndexLSM>-1 || dotIndexMha>-1 || dotIndexzip>-1 || dotIndexVNC>-1 || dotIndexV3raw>-1 || dotIndexV3>-1 || dotIndex>-1){
			...
			if(PathExt==1){
				...
				//Autoscale added by Jiajun Zhang
				title_before_scale = getTitle();
				Stack.getDimensions(width, height, channels, slices, frames);
				if (width>height && width!=1210) {
					run("Scale...", "x=- y=- z=- width=1210 height=566 depth=174 interpolation=Bilinear average process create");
					close(title_before_scale);
					rename(title_before_scale);
				}
				else if (width<height && height!=1209) {
					run("Scale...", "x=- y=- z=- width=573 height=1119 depth=219 interpolation=Bilinear average process create");
					close(title_before_scale);
					rename(title_before_scale);
				}
				//End Autoscale
		...
		if(nImages>0 && nSlices>1){
			...
			for(MIPtry=1; MIPtry<=imageNum; MIPtry++){
				...
				ColorCoder(slices, applyV, width, AutoBRV, bitd, CLAHE, colorscale, reverse0, colorcoding, usingLUT,DefMaxValue,startMIP,endMIP,expand,GammaON,Hrizontal);
				...
				if(imageNum==1){
					if(AutoBRV==1){
						if(packbits==0){
							...
							rename(origiMIP+".tif");
							run("PackBits save activeIMG", "save="+myDir2Co+"");
							close();
							
							if(Hrizontal){
								selectWindow("H_CDM.tif");
								rename(origiMIP+"_H.tif");
								run("PackBits save activeIMG", "save="+myDir2Co+"");
								close();
							}
						}
					...
					if(AutoBRV==1){
						...
						if(packbits==0){
							...
							rename(origiMIP+"_CH"+MIPtry+".tif");
							...
							if(Hrizontal){
								selectWindow("H_CDM.tif");
								rename(origiMIP+"_CH"+MIPtry+"_H.tif");
								run("PackBits save activeIMG", "save="+myDir2Co+"");
								close();
							}
						}
...
function ColorCoder(slicesOri, applyV, width, AutoBRV, bitd, CLAHE, GFrameColorScaleCheck, reverse0, colorcoding, usingLUT,DefMaxValue,startMIP,endMIP,expand,GammaON,Hrizontal) {//"Time-Lapse Color Coder" 
	...
	if(Hrizontal){
		selectWindow("Original_Stack.tif");
		run("Duplicate...", "title=Hstack.tif duplicate");
		
		selectWindow("Hstack.tif");
		makeRectangle(0, 21, 1210, 513);
		run("Crop");
		run("Reslice [/]...", "output=0.5189161 start=Top");
		rename("resliced.tif");
		
		run("Make Substack...", "  slices=1-256");
		rename("upper.tif");
		getDimensions(width, height, channels, slices, frames);
		slicesOri=slices;
		...
		run(Glut);
		run("RGB Color");
		selectWindow("upper.tif");
		run("Z Code Stack HO", "data=upper.tif 1px=lut_table.tif");
		...
		if (GFrameColorScaleCheck==1){
			CreateScale(Glut, Gstartf, slicesOri, reverse0,Hrizontal);
		...
		if (GFrameColorScaleCheck==1){
			...
			makeRectangle(width-257, height-40, 256, 41);
			run("Paste");
			...
			if(AutoBRV==1){
				...
				if(applyV>999 && applyV<10000){
					...
					drawString("Max: 0"+applyV+" /65535", width-210, height-39);
					
					if(bitd==16 && DefMaxValue<=4095)
					drawString("Max: "+applyV+" /4095", width-180, height-39);
					
				}else if(applyV>99 && applyV<1000){
					if(bitd==8)
					drawString("Max: "+applyV+" /255", width-180, height-39);
					
					if(bitd==16 && DefMaxValue>4095)
					drawString("Max: 00"+applyV+" /65535", width-210, height-39);
					
					if(bitd==16 && DefMaxValue<=4095 && DefMaxValue>255)
					drawString("Max: 0"+applyV+" /4095", width-180, height-39);
					
					if(bitd==16 && DefMaxValue<=255)
					drawString("Max: 0"+applyV+" /255", width-180, height-39);
					
				}else if(applyV<100){
					if(bitd==8)
					drawString("Max: 0"+applyV+" /255", width-180, height-39);
					if(bitd==16 && DefMaxValue<=4095 && DefMaxValue>=256)
					drawString("Max: 00"+applyV+" /4095", width-180, height-39);
					if(bitd==16 && DefMaxValue>4095)
					drawString("Max: 000"+applyV+" /65535", width-210, height-39);
					if(bitd==16 && DefMaxValue<=255)
					drawString("Max: 0"+applyV+" /255", width-180, height-39);
					
				}else if(applyV>9999){
					drawString("Max: "+applyV+" /65535", width-210, height-39);
				}
		...
		selectWindow("resliced.tif");
		
		run("Make Substack...", "  slices=257-512");
		rename("down.tif");
		run("Reverse");
		
		run("Z Code Stack HO", "data=down.tif 1px=lut_table.tif");
	...
	//		aa
		...
		run("Combine...", "stack1=color_up.tif stack2=color_down.tif combine");
		rename("H_CDM.tif");
	...
	rename("color.tif");
	if (GFrameColorScaleCheck==1){
		CreateScale(Glut, Gstartf, slicesOri, reverse0,0);
	...
	run("Properties...", "channels=1 slices=1 frames=1 unit=pixel pixel_width=1.0000 pixel_height=1.0000 voxel_depth=1.000");
...
function CreateScale(lutstr, beginf, endf, reverse0,Hrizontal){
	ww = 256;
	hh = 32;
	if(Hrizontal)
	hh = 25;
